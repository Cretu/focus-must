...
- `src/main.ts`：应用入口
- `src/App.vue`：主界面与核心状态编排（planning / settings / focus）
- `src/components/HistoryList.vue`：历史记录展示与筛选
- `src/composables/useBreakTimer.ts`：休息倒计时逻辑
- `src/composables/useSnowEffect.ts`：背景雪花动画
...
- `settings.json`：用户设置（默认白名单）
- `sessions.jsonl`：历史会话（每行一个 JSON 记录）
